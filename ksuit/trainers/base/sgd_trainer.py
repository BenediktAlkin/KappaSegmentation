...
class SgdTrainer:
    def __init__(
        ...
        if self.initializer is None:
            ...
            assert self.start_checkpoint.epoch * self.updates_per_epoch == self.start_checkpoint.update, \
        ...
        for batch_size in reversed(batch_sizes):
            ...
            try:
                for _ in range(2):
                    ...
                    # backup batch because trainer might call batch.pop which depleats the batch
                    og_batch = {**batch}
                    self.update(
                        model=model,
                        ddp_model=ddp_model,
    ...
    def wrap_ddp(self, trainer_model):
        if is_distributed():
            if get_trainable_param_count(trainer_model) > 0:
                ...
                device = trainer_model.model.device
            if device != torch.device("cpu") and self.sync_batchnorm:
