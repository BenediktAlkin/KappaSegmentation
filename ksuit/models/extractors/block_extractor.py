...
class BlockExtractor(ExtractorBase):
    def __init__(self, block_index=None, block_indices=None, num_last_blocks=None, use_pre_blocks=False, **kwargs):
        ...
        self.use_pre_blocks = use_pre_blocks
    ...
    def _register_hooks(self, model):
        ...
        if self.use_pre_blocks:
            hook = ForwardHook(
                outputs=self.outputs,
                raise_exception=self.raise_exception and len(self._resolved_block_indices) == 0,
                **self.hook_kwargs,
            )
            model.pre_blocks.register_forward_hook(hook)
