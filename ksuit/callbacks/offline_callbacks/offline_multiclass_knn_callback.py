...
class OfflineMulticlassKnnCallback(PeriodicCallback):
    ...
    def _before_training(self, model, **kwargs):
        ...
        # make sure that only last extractor can throw StopForwardException
        for non_raising_extractor in self.extractors[:-1]:
            assert not non_raising_extractor.raise_exception
    ...
    def _forward(self, batch, trainer_model, trainer):
        ...
        for extractor in self.extractors:
            ...
            if not torch.is_tensor(extracted):
                assert isinstance(extracted, list)
                assert len(extracted) == 1
                extracted = extracted[0]
                assert torch.is_tensor(extracted)
            extracted = extracted.cpu()
            features[str(extractor)] = extracted
